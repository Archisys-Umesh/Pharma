...
/**
 * Base class that represents a row from the 'unitmaster' table.
 *
 *
 *
...
abstract class Unitmaster implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\entities\\Map\\UnitmasterTableMap';
    ...
    /**
     * The value for the unit_code field.
     *
    ...
    {
        $this->unit_code = '0';
        $this->unit_description = '0';
    }

    /**
     * Initializes internal state of entities\Base\Unitmaster object.
     * @see applyDefaults()
     */
    ...
    /**
     * Get the [unit_code] column value.
     *
     * @return string
     */
    ...
    public function setUnitId($v)
    {
        ...
        if ($this->unit_id !== $v) {
            $this->unit_id = $v;
            $this->modifiedColumns[UnitmasterTableMap::COL_UNIT_ID] = true;
        }
    ...
    /**
     * Set the value of [unit_code] column.
     *
     * @param string $v New value
     * @return $this The current object (for fluent API support)
     */
    public function setUnitCode($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }
        ...
        if ($this->unit_code !== $v) {
            $this->unit_code = $v;
            $this->modifiedColumns[UnitmasterTableMap::COL_UNIT_CODE] = true;
        }
    ...
    public function setUnitDescription($v)
    {
        ...
        if ($this->unit_description !== $v) {
            ...
            $this->modifiedColumns[UnitmasterTableMap::COL_UNIT_DESCRIPTION] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : UnitmasterTableMap::translateFieldName('UnitId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->unit_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : UnitmasterTableMap::translateFieldName('UnitCode', TableMap::TYPE_PHPNAME, $indexType)];
            $this->unit_code = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : UnitmasterTableMap::translateFieldName('UnitDescription', TableMap::TYPE_PHPNAME, $indexType)];
            $this->unit_description = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : UnitmasterTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UnitmasterTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = UnitmasterTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\entities\\Unitmaster'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if (null === $this->unit_id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('unitmaster_unit_id_seq')");
                $this->unit_id = (int) $dataFetcher->fetchColumn();
            } catch (Exception $e) {
        ...
        if ($this->isColumnModified(UnitmasterTableMap::COL_UNIT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'unit_id';
        }
        if ($this->isColumnModified(UnitmasterTableMap::COL_UNIT_CODE)) {
            $modifiedColumns[':p' . $index++]  = 'unit_code';
        }
        if ($this->isColumnModified(UnitmasterTableMap::COL_UNIT_DESCRIPTION)) {
            $modifiedColumns[':p' . $index++]  = 'unit_description';
        }
        ...
        $sql = sprintf(
            'INSERT INTO unitmaster (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'unit_id':
                        ...
                        break;
                    case 'unit_code':
                        $stmt->bindValue($identifier, $this->unit_code, PDO::PARAM_STR);

                        break;
                    case 'unit_description':
                        $stmt->bindValue($identifier, $this->unit_description, PDO::PARAM_STR);
    ...
    public function toArray(string $keyType = TableMap::TYPE_PHPNAME, bool $includeLazyLoadColumns = true, array $alreadyDumpedObjects = [], bool $includeForeignObjects = false): array
    {
        ...
        $result = [
            ...
            $keys[1] => $this->getUnitCode(),
            $keys[2] => $this->getUnitDescription(),
    ...
    public function fromArray(array $arr, string $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setUnitCode($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setUnitDescription($arr[$keys[2]]);
        }
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(UnitmasterTableMap::COL_UNIT_ID)) {
            $criteria->add(UnitmasterTableMap::COL_UNIT_ID, $this->unit_id);
        }
        if ($this->isColumnModified(UnitmasterTableMap::COL_UNIT_CODE)) {
            $criteria->add(UnitmasterTableMap::COL_UNIT_CODE, $this->unit_code);
        }
        if ($this->isColumnModified(UnitmasterTableMap::COL_UNIT_DESCRIPTION)) {
            $criteria->add(UnitmasterTableMap::COL_UNIT_DESCRIPTION, $this->unit_description);
        }
        if ($this->isColumnModified(UnitmasterTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param object $copyObj An object of \entities\Unitmaster (or compatible) type.
    ...
    {
        $copyObj->setUnitCode($this->getUnitCode());
        $copyObj->setUnitDescription($this->getUnitDescription());
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \entities\Unitmaster Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    public function setCompetitionMappings(Collection $competitionMappings, ?ConnectionInterface $con = null)
    {
        ...
        foreach ($competitionMappingsToDelete as $competitionMappingRemoved) {
            $competitionMappingRemoved->setUnitmaster(null);
        }
    ...
    protected function doAddCompetitionMapping(ChildCompetitionMapping $competitionMapping): void
    {
        ...
        $competitionMapping->setUnitmaster($this);
    }
    ...
    public function removeCompetitionMapping(ChildCompetitionMapping $competitionMapping)
    {
        if ($this->getCompetitionMappings()->contains($competitionMapping)) {
            ...
            $competitionMapping->setUnitmaster(null);
        }
    ...
    public function setOrderliness(Collection $orderliness, ?ConnectionInterface $con = null)
    {
        ...
        foreach ($orderlinessToDelete as $orderlinesRemoved) {
            $orderlinesRemoved->setUnitmaster(null);
        }
    ...
    public function setProductss(Collection $productss, ?ConnectionInterface $con = null)
    {
        ...
        foreach ($productssToDelete as $productsRemoved) {
            $productsRemoved->setUnitmaster(null);
        }
