...
abstract class Tags implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\entities\\Map\\TagsTableMap';
    ...
    /**
     * Initializes internal state of entities\Base\Tags object.
     */
    ...
    public function setTagName($v)
    {
        ...
        if ($this->tag_name !== $v) {
            $this->tag_name = $v;
            $this->modifiedColumns[TagsTableMap::COL_TAG_NAME] = true;
        }
    ...
    public function setCompanyId($v)
    {
        ...
        if ($this->company_id !== $v) {
            $this->company_id = $v;
            $this->modifiedColumns[TagsTableMap::COL_COMPANY_ID] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TagsTableMap::translateFieldName('TagName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->tag_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : TagsTableMap::translateFieldName('CompanyId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = TagsTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\entities\\Tags'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('tags_id_seq')");
        ...
        if ($this->isColumnModified(TagsTableMap::COL_TAG_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'tag_name';
        }
        if ($this->isColumnModified(TagsTableMap::COL_COMPANY_ID)) {
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(TagsTableMap::COL_TAG_NAME)) {
            $criteria->add(TagsTableMap::COL_TAG_NAME, $this->tag_name);
        }
        if ($this->isColumnModified(TagsTableMap::COL_COMPANY_ID)) {
            $criteria->add(TagsTableMap::COL_COMPANY_ID, $this->company_id);
        }
     ...
     * objects.
     *
     * @param object $copyObj An object of \entities\Tags (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \entities\Tags Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
